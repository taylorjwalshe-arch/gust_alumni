...
export default function DirectoryDetailView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const candidate = toPerson((json && (json.item as unknown)) ?? json) ?? null;
  ...
  const name = [person.firstName, person.lastName].filter(Boolean).join(" ").trim() || "Unnamed";
  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      {person.location && <div style={{ color: "#6b7280" }}>{person.location}</div>}
    </div>
  );
}
