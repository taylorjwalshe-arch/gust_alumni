...
export default function Header() {
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 shadow-md bg-white">
      <Link href="/" className="font-bold text-lg">Alumni Connect</Link>
      <div className="flex items-center gap-4">
        ...
        {me.authed ? (
          <div className="flex items-center gap-2">
            ...
            {me.role ? <span className="text-xs px-2 py-1 bg-gray-200 rounded-full">{me.role}</span> : null}
            <Link href="/api/auth/signout" className="text-blue-600 text-sm">Sign out</Link>
          </div>
        ) : (
          <Link href="/api/auth/signin" className="text-blue-600 text-sm">Sign in</Link>
        )}
      </div>
    </header>
  );
